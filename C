...
\x1b[0;30m   (_____  )| |   | || |(_)| |   | |   
\x1b[0;30m         ) || |   | || |   | |   | |   
\x1b[0;30m    /\____) || (___) || )   ( |___) (___
\x1b[0;30m   \_______)(_______)|/     \|\_______/
\033[1;97m-----------------------------------------------
\x1b[0;36mDEVOLPER    : SOMI AWAN
\x1b[0;36mWHATSAAP    : 03455453538
\x1b[0;36mFACEBOOK    : SO-MI
\033[1;97m-----------------------------------------------
"""
def reg():
    ...
    time.sleep(1)
    try:
        to = open('/sdcard/.somi.txt', 'r').read()
    except (KeyError, IOError):
        reg2()

    r = requests.get('https://raw.githubusercontent.com/SOMI-BRAND/pro/main/.server.txt').text
    if to in r:
        ...
        print ''
        print ' \x1b[1;92mCopy the id and send to Somi '
        print ''
...
def a_s():
    ...
    if a_s == '1':
		...
		try:
			seat = requests.get('https://graph.facebook.com/' + idt + '/friends?access_token=' + token)
		...
		os.save('/sdcard/friends_id.txt','out/'+done)
